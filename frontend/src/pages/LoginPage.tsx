...
export const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      toast('Login Failed', err.response?.data?.error?.message || 'Invalid credentials provided', 'error');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          <div className="w-14 h-14 rounded-2xl bg-[#2F4F3A] flex items-center justify-center text-white font-bold text-2xl shadow-md mb-3">
            <Sparkles className="w-7 h-7 text-[#DCE9DE]" />
          </div>
          <h1 className="text-2xl font-bold text-[#27332B] tracking-tight">SGC CRM</h1>
          <p className="text-xs font-bold text-[#5E8C61] mt-1 uppercase tracking-wider">Enterprise Management Platform</p>
        </div>

        {/* Card */}
        <div className="bg-white rounded-2xl p-6 md:p-8 shadow-[0_6px_20px_rgba(47,79,58,.05)] border border-[#E3E8E3]">
          <div className="mb-6">
            <h2 className="text-lg font-bold text-[#27332B]">Sign in to your account</h2>
            <p className="text-xs text-[#6B7280] mt-1">Enter your corporate credentials to access your dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button
              type="submit"
              variant="primary"
              className="w-full mt-3 py-2.5 bg-[#2F4F3A] hover:bg-[#243E2E] text-white shadow-xs transition-all"
              isLoading={isLoading}
